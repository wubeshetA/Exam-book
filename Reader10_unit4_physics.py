...
class Question_reader:
    ...
    def __init__(self, master, question_file, answer_file, AnswerIndex):
        ...
        self.me = 'wubshet'
        self.submit_btn = Button(self.tops, text='submit', bg='DeepSkyBlue4', fg='white', padx=10,
                                 font=('Calibri (Body)', 12, 'bold'),
        ...
        p1 = PhotoImage(file='grade10\\physics\\wire&field.png')
        l = Label(scroll_frame, image=p1)
        l.place(x=200, y=1410)
        l.image = p1
        # =============
        p2 = PhotoImage(file='grade10\\physics\\E&P.png')
        l2 = Label(scroll_frame, image=p2)
        l2.place(x=480, y=2170)
        l2.image = p2
    ...
    def show_result(self):
        ...
        bottom_f.pack()
        self.okBtn = Button(bottom_f, text='ok', width=15, bg='DeepSkyBlue4', fg='white',
                            font=('Calibri (Body)', 12, 'bold'), relief=FLAT, command=self.result_win_destroy)
        ...
        Label(bottom_f, text='Result: %d/%d' % (mark, all_q), font=('Courier New', 15, 'bold')).grid(column=0, row=0,
                                                                                                     padx=13)
    ...
    def makeTimeButton(self):
        ...
        self.makeWidgets()
        self.start = Button(self.tops, text='Start', command=self.Start, bg='DeepSkyBlue4', fg='white',
                            font=('Calibri (Body)', 12, 'bold'), relief=FLAT, padx=20)
        self.start.grid(column=1, row=1, padx=5)
        self.stop = Button(self.tops, text='Stop', command=self.Stop, state=DISABLED, bg='DeepSkyBlue4', fg='white',
                           font=('Calibri (Body)', 12, 'bold'), relief=FLAT, padx=20)
        self.stop.grid(column=2, row=1, padx=5)
        self.reset = Button(self.tops, text='Reset', command=self.Reset, state=DISABLED, bg='DeepSkyBlue4', fg='white',
                            font=('Calibri (Body)', 12, 'bold'), relief=FLAT, padx=20)
...
#root = Tk()
#app = Question_reader(root, 'grade10\\physics\\unit4_ques.txt', 'grade10\\physics\\unit4_ans.txt',[0, 7, 11, 12, 19, 20, 27, 30, 32, 36, 42, 47, 50, 55, 59])
#root.mainloop()
